...
func Setup(ctx context.Context, cli *client.Client, c *Config) {
	...
	if viper.GetBool("defaults") {
		...
		var ResolvMacOS = resolv.Resolv{
			Data:    fmt.Sprintf("# Generated by pygmy\nnameserver 127.0.0.1\ndomain %s\nport 6053\n", c.Domain),
			Enabled: true,
			File:    c.Domain,
			Folder:  "/etc/resolver",
			Name:    "MacOS Resolver",
	...
	if c.Defaults {
		...
		ImportDefaults(ctx, cli, c, "pygmy-ssh", agent.New())
		ImportDefaults(ctx, cli, c, "pygmy-ssh-add-key", key.NewAdder())
		ImportDefaults(ctx, cli, c, "pygmy-dns", dnsmasq.New(&dockerruntime.Params{Domain: c.Domain}))
		ImportDefaults(ctx, cli, c, "pygmy-proxy", haproxy.New(&dockerruntime.Params{Domain: c.Domain}, c.TLSCertPath))
		ImportDefaults(ctx, cli, c, "pygmy-mail", mailhog.New(&dockerruntime.Params{Domain: c.Domain}))
		...
		// If a port on pygmy-proxy isn't explicitly declared,
		// then we should set this value. This is far more creative
		// than needed, so feel free to revisit if you can compile it.
		if c.Services["pygmy-proxy"].HostConfig.PortBindings == nil {
			c.Services["pygmy-proxy"] = GetService(haproxy.NewDefaultPorts(), c.Services["pygmy-proxy"])
		}
		...
		if c.Services["pygmy-mail"].HostConfig.PortBindings == nil {
			c.Services["pygmy-mail"] = GetService(mailhog.NewDefaultPorts(), c.Services["pygmy-mail"])
		}
