...
RUN GO111MODULE=on GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -a -ldflags '-extldflags "-static"' -o pygmy-go-linux-arm-static .
...
COPY --from=builder /go/src/github.com/fubarhouse/pygmy-go/pygmy-go-linux-static .
